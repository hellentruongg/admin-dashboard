import Toplist from "../../components/cards/Toplist";
import "./Home.scss";

const Home = () => {
  return (
    <div className="overview">
      <article className="overview__card overview__1">
        <Toplist />
      </article>
      <article className="overview__card overview__2">Card 2</article>
      <article className="overview__card overview__3">Card 3</article>
      <article className="overview__card overview__4">Card 4</article>
      <article className="overview__card overview__5">Card 5</article>
      <article className="overview__card overview__6">Card 6</article>
      <article className="overview__card overview__7">Card 7</article>
      <article className="overview__card overview__8">Card 8</article>
      <article className="overview__card overview__9">Card 9</article>
    </div>
  );
};

export default Home;
